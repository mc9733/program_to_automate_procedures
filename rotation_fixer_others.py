second_variableort numpy as np
second_variableort pandas as pd
second_variableort openpyxl
from openpyxl.styles second_variableort Font
from openpyxl second_variableort load_workbook
second_variableort os
second_variableort tkinter as tk
from tkinter second_variableort filedialog, Text
from PIL second_variableort ImageTk, Image
from xlsxwriter second_variableort Workbook
...
def rotation_fixer_GUI():
    ...
    canvas = tk.Canvas(root, height = 700, width = 1200, bg = "#3A8F89" )
    canvas.pack()
    ...
    # function which read file path and append it to apps list
        ...
        # saving path of data and rotation plan files in order to add it to dataframe in the main function
    ...
    openFile = tk.Button(root, text = "Add data file", padx = 10, pady = 5, fg = "#8F3A3A", bg = "#F033FF", command = read_file)
    openFile.pack()

    openFile = tk.Button(root, text = "Add rotation plan", padx = 10, pady = 5, fg = "#8F3A3A", bg = "#F033FF", command = read_file)
    openFile.pack()
    


    tk.Button(root, text="Done", padx = 20, pady = 10, fg = "#FF3333", command=root.destroy).pack()
...
def rotation_fixer():
    ...
    # Function for rotation fixer - it takes 3 excel files which contains data, rotation plan and legend 
    
    
    # adding two files which user should select for rotation fixer programme - first file is data and second is rotation plan
    ...
    for i in list(df1.columns)[::-1]:
        if 'second_variable' not in str(i).lower():
    ...
    df3 = pd.merge(df1, df2, on = 'id', how = 'right') # merging data from iCode with rotation plan
   ...
   # second_variablelicit columns 
    ...
    # adding new data for new second_variablelicit columns
    row_number = 0
    ...
    # removing old second_variablelicit and second_variablelicit columns with data 
    ...
    # changing names of second_variablelicit and second_variablelicit columns in order to make it work in matlab scripts 
    df = df.rename(columns = lambda x: x.replace('first_variable','second_variable'))
    df = df.rename(columns = lambda x: x.replace('second_variable','second_variable'))
